...
metadata: {"clawdbot":{"emoji":"🧠","requires":{"bins":[]},"os":["linux","darwin","win32"],"configPaths":["./data/"]}}
---
...
Memory lives in `./data/` with tiered structure. If `./data/` does not exist, run `setup.md`.

```
./
├── data/              # Dynamic storage (memory, index, logs)
├── templates/         # Read-only structural templates (*.template.md)
├── SKILL.md           # This definition
...
- Stores preferences in local files (`./data/`)
- Reads its own memory files on activation

This skill NEVER:
- Accesses calendar, email, or contacts
- Makes network requests
- Reads files outside `./data/`
